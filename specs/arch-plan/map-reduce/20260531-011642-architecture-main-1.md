...
Define the shared structure for bounded parallel parsing in `stacklit generate-json` while preserving deterministic output, one-worker compatibility, and explicit worker-count rollout controls.
...
- Store benchmark evidence as a repo-tracked markdown artifact under `specs/benchmarks/map-reduce/`. Rationale: benchmark results are acceptance evidence, not runtime behavior, and need reviewable provenance.
...
**Contract:** Benchmark evidence records wall time and max RSS for worker counts 1, 2, 4, and 8 on the same large repository state before any future default above one is proposed.

**Direction:** Documentation and benchmark report consume the implemented controls.

**Invariants:** Benchmarking does not change runtime schema, graph semantics, or default worker count in this goal.
...
| Observability | Existing quiet/non-quiet status output remains unchanged except parsed count reflects parser results; benchmark evidence records wall time and max RSS externally. |
...
| Testing | Parser tests cover one-worker and multi-worker behavior, parse-error collection, deterministic ordering, and race detector; engine/CLI/config tests cover resolution, override precedence, invalid values, and output equivalence; docs/benchmark scope covers rollout evidence. |
...
The decomposition keeps concurrency ownership inside the parser map stage, keeps configuration and precedence in the existing CLI/config/engine path, and keeps rollout evidence separate from runtime behavior. The main load-bearing decision is the parser/engine interface: downstream scopes must treat it as the single contract for worker-count-aware parsing rather than redefining concurrency at the CLI or engine layer.

---

## Decomposition

Each scope becomes a code-planning child task. Scope 0 is emitted only because this worktree lacks `.pre-commit-config.yaml` and no active repo-wide `bootstrap-precommit` task exists.
